		...
		<a-card :bordered="false" class="header-solid mb-24">
			<div class="top">
				<div style="margin-bottom: 15px;">
					<span class="divide-span" style=" margin-right: 10px;">审批状态：</span>
					<a-select :options="inputTypeList" placeholder="流程选择" v-model="searchInfo.currentStatus" />
				</div>
			...
			<div class="count">
				<a-table :columns="columns" :data-source="data" :row-selection="rowSelection" @change="tableChange"
					...
					<span slot="action" slot-scope="text, record">
						<span
							v-if="$hasPer(['system']) && (record.currentStatusName =='待审批' || record.currentStatusName =='审批中')"
							style="color: #028BE2; cursor:pointer; margin-right: 20px;"
							@click="joinStu('change',record.id)">审批</span>
						<span
							v-if="$hasPer(['system'])" style="color: #028BE2; cursor:pointer; margin-right: 20px;"
							@click="joinlogs('change',record.id)">审批记录
						</span>	
					</span>
				</a-table>
			</div>
			<div class="showNewDoginBox">
				...
				<joinOrChangeStulogs ref="joinOrChangeStulogs" @JoinOrchangeStuSuss='JoinOrchangeStuSuss'>
				</joinOrChangeStulogs>
				<!-- </a-modal> -->
			</div>
		</a-card>
...
<script>
	...
	import joinOrChangeStulogs from './logs.vue'
	import {
		mapActions,
		mapGetters,
		mapState
	...
	export default {
		...
		methods: {
			...
			remoder() {
				this.searchInfo.currentStatus = undefined
				this.getStuInfo()
			},
			joinStu(type, id) {
			...
			getStuInfo() {
				let _this = this;
				this.$http.get('/sys/processInstance/pendingApprovals', this.searchInfo)
					.then(res => {
						if (res.code == 0) {
