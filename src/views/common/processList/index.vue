			...
			<div class="count">
				<a-table :columns="columns" :data-source="data" :row-selection="rowSelection" @change="tableChange"
					...
					<span slot="action" slot-scope="text, record">
						<span v-if="$hasPer(['system']) && record.currentStatusName=='待审批'"  style="color: #028BE2; cursor:pointer; margin-right: 20px;" 
							@click="joinStu('change',record.id)">修改</span>
						<span v-if="$hasPer(['system']) && record.currentStatusName=='已拒绝'"  style="color: #028BE2; cursor:pointer; margin-right: 20px;"
							@click="joinStu('regoup',record.id)">重新提交</span>	
					</span>
				</a-table>
			</div>
...
<script>
	...
	export default {
		...
		methods: {
			...
			getStuInfo() {
				let _this=this;
				this.$http.get('/sys/processInstance/list', this.searchInfo)
					.then(res => {
						if (res.code == 0) {
			...
			delteIds() {
				...
				this.$confirm({
					...
					onOk() {
						...
						then.$http.delete('/sys/processInstance/deleteBatch?ids=' + ids)
							.then(res => {
