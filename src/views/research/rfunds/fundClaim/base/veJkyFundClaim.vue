<template>
  ...
  <div class="right_table">
    ...
    <!-- 编辑页 -->
    <veJkyFundClaimEdit v-if="showType == 'edit'" ref="veJkyFundClaimEdit" @onOk="editOk" @onCancel="editCancel"> </veJkyFundClaimEdit>
    <veJkyFundClaimShow v-if="showType == 'show'" ref="veJkyFundClaimShow" @onOk="editOk" @onCancel="editCancel"> </veJkyFundClaimShow>
  </div>
</template>

<script>
import veJkyFundClaimApi from '@/api/research/rfunds/veJkyFundClaimApi';
import veJkyFundClaimEdit from './veJkyFundClaimEdit';
import veJkyFundClaimShow from './veJkyFundClaimShow';
...
export default {
  ...
  methods: {
    ...
    doAudit(row, status) {
      this.msgUtil.confirm('审核提示', `确定要 [${status == 1 ? '通过' : '不通过'}] 选择的项目 [${row.projectName}] 的报销吗?`, () => {
        veJkyFundClaimApi
          .updateStatus({
            ids: [row.id],
            status: status
          ...
          .then(res => {
            this.$notification.success({
              message: '数据操作成功',
              description: `成功${status == 1 ? '通过' : '不通过'} 项目 [${row.projectName}] 的报销`
            });
            this.handleFilter();
          })
          .catch(res => {
    ...
    handleCreate() {
      this.showType = 'edit';
      this.$nextTick(() => {
        this.$refs.veJkyFundClaimEdit.doCreate();
      });
    },
    /**
     * @msg: 编辑行事件
    ...
    handleUpdate(row) {
      this.showType = 'edit';
      this.$nextTick(() => {
        this.$refs.veJkyFundClaimEdit.doUpdate(row);
      });
    },
    /**
     * @msg: 编辑页取消事件
