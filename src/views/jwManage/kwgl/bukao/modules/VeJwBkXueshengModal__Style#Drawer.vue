  ...
  <a-drawer
    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        ...
        <a-form-item label="是否已参加补考（0=未参加，1=已参加）录入成绩时，标记为已参加补考" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-input-number v-decorator="[ 'isjoin', validatorRules.isjoin]" placeholder="请输入是否已参加补考（0=未参加，1=已参加）录入成绩时，标记为已参加补考" style="width: 100%"/>
        </a-form-item>
  ...
  export default {
    ...
    data () {
      return {
        ...
        url: {
          add: "/kwgl/veJwBkXuesheng/add",
          edit: "/kwgl/veJwBkXuesheng/edit",
        }
      }
    },
    created () {
    ...
    methods: {
      ...
      edit (record) {
        ...
        this.visible = true;
        this.$nextTick(() => {
          this.form.setFieldsValue(pick(this.model,'ksid','cjid','bkid','falid','specid','xzid','gradeid','semid','kcid','jxbid','xzbid','stuid','userid','xh','xm','isjoin','terminalid','createtime'))
        })
      },
      close () {
        this.$emit('close');
      ...
      popupCallback(row){
        this.form.setFieldsValue(pick(row,'ksid','cjid','bkid','falid','specid','xzid','gradeid','semid','kcid','jxbid','xzbid','stuid','userid','xh','xm','isjoin','terminalid','createtime'))
      }
