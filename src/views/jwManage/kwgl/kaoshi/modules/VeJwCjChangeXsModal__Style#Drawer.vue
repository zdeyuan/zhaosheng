  ...
  <a-drawer
    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        ...
        <a-form-item label="成绩修改记录ID" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-input-number v-decorator="[ 'changeid', validatorRules.changeid]" placeholder="请输入成绩修改记录ID" style="width: 100%"/>
        </a-form-item>
  ...
  export default {
    ...
    data () {
      return {
        ...
        validatorRules: {
          changeid: {rules: [
            {required: true, message: '请输入成绩修改记录ID!'},
          ]},
          ksid: {rules: [
        ...
        url: {
          add: "/kwgl/veJwCjChangeXs/add",
          edit: "/kwgl/veJwCjChangeXs/edit",
        }
      }
    },
    created () {
    ...
    methods: {
      ...
      edit (record) {
        ...
        this.visible = true;
        this.$nextTick(() => {
          this.form.setFieldsValue(pick(this.model,'changeid','ksid','stuid','cjlxid','cjlxname','ratio','score','finalscore','terminalid'))
        })
      },
      close () {
        this.$emit('close');
      ...
      popupCallback(row){
        this.form.setFieldsValue(pick(row,'changeid','ksid','stuid','cjlxid','cjlxname','ratio','score','finalscore','terminalid'))
      }
