      ...
      <div>
        <a-form layout="inline">
          <a-row :gutter="24">
            ...
            <a-col :md="8" :sm="24" v-if="$hasPer(['system','teacher','bzr'])">
              <a-form-item label="学生姓名">
               <a-input placeholder="请输入学生姓名"  v-model="queryParam.xm"></a-input>
              </a-form-item>
            </a-col>
            <a-col :md="8" :sm="24" v-if="$hasPer(['system','bzr'])">
...
<!--    &lt;!&ndash; 操作按钮区域 &ndash;&gt;-->
    <div class="table-operator" v-if="$hasPer(['system','teacher','bzr'])">
    ...
    <div>
      ...
      >
        ...
        <span slot="action" slot-scope="text, record">
          <edu-label-button type="1" title="查看" @click="detail(record)"></edu-label-button>
          <a-divider v-if="record.auditstatus!=1" type="vertical" />
          <edu-label-button type="3" title="编辑" v-if="record.auditstatus!=1" @click="edit(record)"></edu-label-button>
          <a-divider type="vertical" />
          <edu-label-button type="2" v-if="$hasPer(['system','teacher','bzr'])" title="审核" @click="apply(record)"></edu-label-button>
    ...
    <VeJwChengjiAudit ref="applyForm" @ok="modalFormOk"></VeJwChengjiAudit>
  </a-card>
</template>
...
import VeJwChengjiAudit from './modules/VeJwChengjiAudit'
...
export default {
  ...
  data() {
    return {
      ...
      url: {
        ...
        stateBatch: "/kwgl/veJwChengji/stateBatch",
  ...
  methods: {
    ...
    apply(record) {
      ...
      this.$refs.applyForm.title = '审核（' + record.xm + '）成绩信息'
