...
<!--      </div>-->
      ...
      >
        <span slot="action" slot-scope="text, record">
         <a @click="handlePutin(record)">出库</a>
          <!-- <edu-label-button type="3" @click="handlePutin(record)" title="出库"></edu-label-button> -->
          <a-divider type="vertical" />
         <a @click="handleKucun(record)">库存</a>
          <!-- <edu-label-button type="2" @click="handleKucun(record)" title="库存"></edu-label-button> -->
    ...
    <!-- 表单区域 -->
    <jw-jiaocai-putout-modal ref="putModalForm" @ok="modalFormOk"></jw-jiaocai-putout-modal>
...
import JwJiaocaiPutoutModal from './modules/JwJiaocaiPutoutModal.vue'
...
export default {
  ...
  methods: {
    ...
    handlePutin: function (record) {
      ...
      getAction(this.url.queryByJcid, { jcid: record.id }).then((res) => {

        if (res.success) {
          this.$refs.putModalForm.edit(record, res.result)
          this.$refs.putModalForm.title = '教材出库'
      ...
      // ).then((res) => {
      //   if (res.success) {
      //     this.$refs.putModalForm.edit(record,res.record)
      //     this.$refs.putModalForm.title = '教材出库'
      //     this.$refs.putModalForm.disableSubmit = false
