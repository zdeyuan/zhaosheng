<template>
  ...
  >
    ...
    <div class="ant-modal-footer" v-if="multiple">
      ...
      <a-button type="primary" @click="handleOk"> 确定选中 </a-button>
    </div>
  </a-modal>
</template>
...
import { queryFacultyList, querySpecialtyListByFalId, queryBanJiListBySpecId,getStudentPageList } from '@/api/base/common-veCommon'
...
export default {
  ...
  created() {
    this.getFacultyList()
    if (this.multiple) {
      this.selectedRowKeys = this.selectVals.map((item) => item.stuid)
      this.selectedRows = this.selectVals
    }
  },
  computed: {
    rowSelection() {
  ...
  methods: {
    ...
    handleOk() {
      this.$emit('ok', this.selectedRowsCopy)
    },
    removeSameItem(arr, key) {
    ...
    onSelectChange(selectedRowKeys, selectedRows) {
      ...
      if (this.multiple) {
        ...
        let _rows = selectedRows.map((item) => {
          return { stuid: item.id, stuname: item.xm }
        })
        this.selectedRows = this.selectedRows.concat(_rows)
        let _selectedRows = this.removeSameItem(this.selectedRows, 'stuid')
        let _arr = []
