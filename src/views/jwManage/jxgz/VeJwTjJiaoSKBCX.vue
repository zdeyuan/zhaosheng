...
<!--                      <a-button type="primary"  icon="printer">打印</a-button>-->
          ...
          <div class="schedule-main">
            ...
            <div class="schedule-main__table">
              ...
              <div v-if="!toballoading" style="display: flex;justify-content: center">
                <a-spin />
              </div>
              <a-row v-if="toballoading" class="table-row table-row__body" v-for="item in scheduleNodeMorningData" :key="item.id">
              ...
              <a-row v-if="toballoading" class="table-row table-row__body" v-for="item in scheduleNodeAfternoonData" :key="item.id">
                <a-col :span="3" class="table-td table-section">
                  <div class="table-td__inner">
                    {{ item.section }}
                    <div class="table-section__time">{{ item.kssj }}-{{ item.jssj }}</div>
              ...
              <a-row v-if="toballoading" class="table-row table-row__body" v-for="item in scheduleNodeNightData" :key="item.id">
                <a-col :span="3" class="table-td table-section">
                  <div class="table-td__inner">
                    {{ item.section }}
                    <div class="table-section__time">{{ item.kssj }}-{{ item.jssj }}</div>
  ...
  export default {
    ...
    methods: {
      ...
      bjkb(){
        ...
        let data={
          jsid:this.queryParam.jsid,
          semid:this.queryParam.semid,
        }
        getAction('/jxrw/kbcx/jskb',data).then(res=>{
          console.log(res);
          if(res.success){
            let arr=res.result.data;
            for(var i=0;i<arr.length;i++){
              if(arr[i].section<=4){
                _this.$set(_this.scheduleNodeMorningData[arr[i].section-1],(arr[i].week-1),arr[i]);
