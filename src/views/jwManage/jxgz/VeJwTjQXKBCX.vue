            ...
            <a-table :columns="columns" rowKey="id" :loading="!loading" :data-source="data2" bordered :scroll="{ x: 7300 }">
              <template slot="left" slot-scope="text,record,index">
                <div style="display: flex;justify-content: center;align-items: center;width: 100%">
                  <div style="width: calc(100% / 3);padding: 0 5px;">{{text.zy}}</div>
                  <div style="width: calc(100% / 3);padding: 0 5px;">{{text.nj}}</div>
                  <div style="width: calc(100% / 3);padding: 0 5px;">{{text.kc}}</div>
                </div>
              </template>
        ...
        getAction('/jxrw/kbcx/qxkb',data).then(res=>{
          ...
          if(res.success){
            ...
            arr.forEach(ret=>{
                ...
                json.left.zy=res.result.specialty;
                json.left.nj=res.result.semester;
                json.left.kc=ret.xzbmc;
              json.id=ret.id;
                json.bzr=ret.bzrName?ret.bzrName:'';
                ret.kbs.forEach(a=>{
