<template>
	<div style="background:#E9EDF6; padding:20px; margin-top:20px;">
		...
		<div class="pageContentBox">
			<div class="headTop"><span class="notTop">成绩公示</span></div>
			<hr class="right-hr">
			<div class="content-head">

				<div>
				
					<span class="head-span">准考证号</span>
					 <!-- 准考证号输入框 -->
                    <a-input class="input-style-replyTwo"  v-model="zkzh"  placeholder="请输入"></a-input>
...
<script>
	...
	export default {
			...
			return {
				...
				url: {
        			list: 'enroll/stuInfoQuery/exportModel',
          			exportXlsUrl: 'enroll/admission/exportScore',
              		importExcelUrl: 'enroll/admission/importScore',
        		},
				pagination: {
					current: 0,
					pageSize: 8,
		...
		methods: {
			getModel(){
				this.url.exportXlsUrl='enroll/admission/getScoreTemplate';
				this.handleExportXls('成绩公示导入模板');
				this.url.exportXlsUrl='enroll/admission/exportScore';
			},
			exportMessage(){
				...
				this.handleExportXls("成绩公示表",conditStr)
			},
            showUpload() {
            this.upload = true
            },
			...
			getScoreList(){
				...
				axios({
					url: 'enroll/admission/getScoreList',
					method: 'post',
					params: {
						 "ZKZH": this.zkzh,
						 "XM": this.stuName,
					...
					this.$message.warning("获取成绩公示列表失败");
				})
			},
			empty(){
