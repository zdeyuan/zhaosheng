      ...
      <div>
        ...
        >
          ...
          <span slot="action" slot-scope="text, record">
            <a @click="jumpToDetail(record)" class="blue-txt">人事变动管理</a>
          </span>
        </a-table>
      </div>
    </div>
    <div v-else>
      <ve-hr-change-person v-model="jobNumber"  :teacherid="teacherid" :extra="extraModel"/>
    </div>

    <!-- <HqHrJiangliModal ref="modalForm" @ok="modalFormOk"></HqHrJiangliModal> -->
...
import VeHrChangePerson from './VeHrChangePerson.vue'
...
export default {
  ...
  data() {
    return {
      ...
      jobNumber: '',
      teacherid:'',
      tabPaths:['人事变动管理','人事变动管理'],
      tabPathsPerson:['人事变动管理','人事变动管理','详情'],
      // 表头
      columns: [
        {
          title: '序号',
  ...
  methods: {
    jumpToDetail(record) {
      ...
      //todo 只有这个的添加需要有id，不然会添加失败，有点奇怪
      console.log(record)
      this.extraModel={
        xm:record.xm,
        gh:record.gh,
