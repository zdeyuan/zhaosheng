      ...
      <div>
        ...
        >
          ...
          <span slot="action" slot-scope="text, record">
            <a class="blue-txt" @click="jumpToDetail(record)">合同管理</a>
          </span>
        </a-table>
      </div>
    </div>
    <div v-else>
      <VeHrHetongPersonalList v-model="jobNumber"  :teacherid="teacherid" :extra="extraModel"/>
    </div>
  </div>
</template>
...
import VeHrHetongPersonalList from './VeHrHetongPersonalList'

export default {
  mixins: [EduListMixin, mixinDevice],
  components: { VeHrHetongPersonalList },
  data() {
    return {
      description: '教职工信息页面',
      isPerson: false,
