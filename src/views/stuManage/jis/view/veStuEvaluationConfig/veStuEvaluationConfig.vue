 ...
 * @LastEditTime: 2021-04-22 20:49:30
-->
<template>
        ...
        <table class="my_table">
          ...
          <tbody>
            <tr v-for="item in dataList" :key="item.id">
              <td>{{ item.name }}</td>
              <td>
                <a-input-number v-model="item.listSort" placeholder="排序" style="width:300px" />
              </td>
              <td>
...
export default {
  ...
  methods: {
    init() {
      ...
      veStuEvaluationConfigApi.all(sub).then(res => {
     ...
     */
    updateData() {
      veStuEvaluationConfigApi.update(this.dataList).then(() => {
