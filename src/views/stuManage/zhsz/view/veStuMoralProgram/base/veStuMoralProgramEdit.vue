<template>
  <div class="edit_form">
    ...
    <edu-form :title="textMap[dialogStatus]" @ok="dialogStatus === 'create' ? createData() : updateData()" @cancel="handelCancel">
      <a-form-model ref="veStuMoralProgramDataForm" :rules="veStuMoralProgramRules" :model="veStuMoralProgramVo" labelAlign="right">
        <a-form-model-item label="计划标题" prop="title" required>
          <a-input v-model="veStuMoralProgramVo.title" :maxLength="200" placeholder="请输入计划标题" />
        </a-form-model-item>
        <a-form-model-item label="内容" prop="content">
          <ueditor ref="ueditor" style="width: 100%%;"></ueditor>
        </a-form-model-item>
        <a-form-model-item label="全部教师" prop="isAll" required>
          <edu-data v-model="veStuMoralProgramVo.isAll" stype="radio" code="shifou" />
        </a-form-model-item>
        <a-form-model-item label="接收教师" v-if="veStuMoralProgramVo.isAll == 0" prop="userIds">
          <userSelectBox ref="userSelectBox" :check="true"></userSelectBox>
        </a-form-model-item>

        <a-form-model-item label="发布状态" prop="status" required>
          <edu-data v-model="veStuMoralProgramVo.status" stype="radio" code="statusFb" />
        </a-form-model-item>
      </a-form-model>
    </edu-form>
  </div>
...
export default {
  ...
  data() {
    ...
    const data = {
      veStuMoralProgramVo: this.resetveStuMoralProgramVo(),
      textMap: {
        update: '编辑-德育计划',
        create: '新增-德育计划'
      },
      dialogStatus: 'create',
      veStuMoralProgramRules: {
		  title: [
		    {
		      required: true,
		      message: '请填写计划标题',
  ...
  methods: {
    ...
    doCreate() {
      ...
      this.veStuMoralProgramVo = this.resetveStuMoralProgramVo();
      this.$nextTick(() => {
        this.$refs.veStuMoralProgramDataForm.clearValidate();
      });
    },
    /**
     * @msg: 初始化修改
     */
    doUpdate(row) {
      veStuMoralProgramApi.get(row.id).then(res => {
        this.veStuMoralProgramVo = res.result;
     ...
     */
    getSubData(isUpdate) {
      const tempData = Object.assign({}, this.veStuMoralProgramVo);
     ...
     */
    createData() {
      this.$refs.veStuMoralProgramDataForm.validate(valid => {
        if (valid) {
          const subData = this.getSubData(false);
          if (subData == null) {
            return;
          }
          veStuMoralProgramApi.add(subData).then(res => {
    ...
    updateData() {
      this.$refs.veStuMoralProgramDataForm.validate(valid => {
        if (valid) {
          ...
          veStuMoralProgramApi.update(subData).then(() => {
            this.$notification.success({
              message: '数据修改成功',
              description: '修改了一条数据'
            });
