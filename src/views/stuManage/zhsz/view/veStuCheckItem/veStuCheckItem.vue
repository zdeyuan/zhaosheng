<template>
  ...
  <div slot="table" class="right_table">
    ...
    <veStuCheckItemNormEdit v-if="showType == 'item'" ref="veStuCheckItemNormEdit" @onOk="editOk" @onCancel="editCancel"> </veStuCheckItemNormEdit>
    <veStuCheckJudgerEdit v-if="showType == 'tea'" ref="veStuCheckJudgerEdit" @onOk="editOk" @onCancel="editCancel"> </veStuCheckJudgerEdit>
    <veStuCheckStudentEdit v-if="showType == 'stu'" ref="veStuCheckStudentEdit" @onOk="editOk" @onCancel="editCancel"> </veStuCheckStudentEdit>
  </div>
</template>
...
import veStuCheckJudgerEdit from './base/veStuCheckJudgerEdit';
import veStuCheckItemNormEdit from './base/veStuCheckItemNormEdit';
import veStuCheckStudentEdit from './base/veStuCheckStudentEdit';

export default {
  components: {
    veStuCheckItemEdit,
    veStuCheckItemShow,
    veStuCheckJudgerEdit,
    veStuCheckItemNormEdit,
  ...
  methods: {
    doSetItem(row) {
      this.showType = 'item';
      this.$nextTick(() => {
        this.$refs.veStuCheckItemNormEdit.doCreate(row);
      });
    },
    doSetStu(row) {
      this.showType = 'stu';
      this.$nextTick(() => {
        this.$refs.veStuCheckStudentEdit.doCreate(row);
      });
    },
    doSetTea(row) {
      this.showType = 'tea';
      this.$nextTick(() => {
        this.$refs.veStuCheckJudgerEdit.doCreate(row);
      });
    },
    /**
