    ...
    <div>
      <edu-query ref="cwQuery" ext="boxZybmbj" :nianji="false" bjStart="s." code="veStuGraduationAuditInfo" :do-filter="handleFilter">
        <template slot="search">
          ...
          <edu-query-item title="审核状态" :defaultFn="() => -1" ref="auditStatusItem" pname="audit_status" code="biyeStatus" pop="eq" stype="select" > </edu-query-item>
        </template>
      </edu-query>
      <edu-table
      ...
      >
        <template slot="af_btn" slot-scope="{ row }">
          ...
          <edu-pms :code="'aaa'" name="审核"  v-if="row.auditStatus == -1||!row.auditStatus" ext="tdo" type="success" size="mini" @click="handleUpdate(row)"> </edu-pms>
          <edu-pms :code="'aaa'" name="修改" v-if="row.auditStatus == 1" ext="tdo" type="success" size="mini" @click="handleUpdate(row)"> </edu-pms>
        </template>
       ...
       <span></span>
        ...
        <a-table-column title="所属专业部" dataIndex="falText" width="200px"> </a-table-column>
...
export default {
  ...
  mounted() {
	  //this.$refs.auditStatusItem.setValue(-1);
  },
  methods: {
    /**
    ...
    handleUpdate(row) {
      this.showType = 'edit';
      this.$nextTick(() => {
        this.$refs.veStuGraduationAuditInfoEdit.doUpdate(row,'BZR');
      });
    },
    /**
     * @msg: 编辑行事件
