<template>
  <div class="edit_form">
    <!-- 编辑页 -->
    <edu-form title="请假信息详情" @cancel="handelCancel" @ok="audit">
      <a-form-model ref="veStuLeaveDataForm" :model="subData" :rules="veStuLeaveRules" labelAlign="right">
        <a-row>
          <a-col :span="12">
        ...
        <a-form-model-item label="审核状态" prop="auditStatus" required>
          <edu-data v-model="subData.auditStatus" stype="radio" code="auditStatus" />
        </a-form-model-item>
        <a-form-model-item v-if="subData.auditStatus == 2" label="未通过原因" prop="auditReason" required>
          <a-textarea v-model="subData.auditReason" :maxLength="300" placeholder="请输入未通过原因" :rows="4" />
        </a-form-model-item>
      </a-form-model>
    </edu-form>
  </div>
