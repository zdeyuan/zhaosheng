...
export function getLodop(oOBJECT, oEMBED) {
    ...
    var strCLodopInstall_1 = "<br><font color='#FF00FF'>Web打印服务CLodop未安装启动，点击这里<a href='http://demo.c-lodop.com:8000/Lodop6.226_Clodop4.127.zip' target='_self'>下载执行安装</a>";
    ...
    try {
        ...
        if (needCLodop()) {
            ...
            if (!LODOP) {
                errmsg = strCLodopInstall_1 + (CLodopIsLocal ? strCLodopInstall_2 : "") + strCLodopInstall_3 ;
                return {errmsg};
            } else {
                if (CLODOP.CVERSION < "4.1.2.3") {
                     errmsg = strCLodopUpdate
					 return {errmsg,LODOP}
                }
            ...
            if ((!LODOP) || (!LODOP.VERSION)) {
                if (ua.indexOf('Chrome') >= 0)
                    errmsg =  strHtmChrome + document.body.innerHTML;
                if (ua.indexOf('Firefox') >= 0)
                    errmsg =  strHtmFireFox + (is64IE ? strHtm64_Install : strHtmInstall) 
                return {errmsg,LODOP};
            }
        }
        if (LODOP.VERSION < "6.2.2.6") {
            if (!needCLodop())
                errmsg = (is64IE ? strHtm64_Update : strHtmUpdate) 
        }
