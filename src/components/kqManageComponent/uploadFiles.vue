  ...
  <div>
    ...
    <div v-show="fileList.length > 0 && showFiles" class="file-table">
      ...
      <a-table rowKey="id" :data-source="fileList" :pagination="false" bordered style="width: 100%">
        <a-table-column title="序号" width="30px" align="center">
          <template slot-scope="text, record, index">
            {{ index + 1 }}
          </template>
        </a-table-column>
        <a-table-column dataIndex="name" v-if="!isImg" width="250px" title="文件名称"> </a-table-column>
        <slot></slot>
        <a-table-column title="操作" width="50px" align="center" dataIndex="id">
...
export default {
  props: {
    url: {
      type: String,
      default:http.service.stu + 'veStuFile/upload'
    },
    name: {
      type: String, //默认的后台名称
      default: 'file'
