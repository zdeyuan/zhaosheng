...
<script>
  import {getActionDictListByModel} from '@/api/common/manage'

  export default {
    name: "JSelectDictKwgl",
    props: {
      dictCode: String,
      placeholder: String,
    ...
    methods: {
      initDictData() {
        //根据字典Code, 初始化字典数组
        getActionDictListByModel(this.dictCode).then((res) => {
          if (res.success) {
            let array =res.result;
            let target = []
            for(let i=0;i<array.length;i++){
              target.push({value:array[i].code,text:array[i].title})
            }
